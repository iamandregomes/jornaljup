	...
	if(!empty($header_sections)) {
		foreach($header_sections as $i => $section ){
			if( $i ){
				...
				if($bg1 == $bg2){
					echo '.header-'.$section.'{ border-top: 0px solid '.herald_hex2rgba(herald_get_option('color_header_'.$section.'_txt'), 0.15).';}';
				}
			}
			...
			if( ($i + 1) == count( $header_sections )){
				...
				if( $last_bg == $color_content_bg ){
					echo '.header-'.$section.'{ border-bottom: 0px solid '.herald_hex2rgba(herald_get_option('color_header_'.$section.'_txt'), 0.15).';}';
				}
